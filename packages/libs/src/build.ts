...
export function destinationResolver() {
	...
	const resolve: FDst = ({assets, dstFile, opts: {ignore, ...opts}}) => {
		...
		for (const asset of files) {
			...
			if (
				list[asset].isMatch(dstFile) &&
				dstFile.slice(0, list[asset].base.length).toUpperCase() ===
					list[asset].base.toUpperCase()
			) {
...
export async function getFiles(
	...
	for (const filepath of files) {
		...
		const indexMatch = data.findIndex((item) =>
			mm.isMatch(filepath, item.input)
		);
		...
		name = output
			? replacePosixSep(
					join(replaceOutput, flatten ? basename(filepath) : _dstFile)
				)
			: _dstFile;
...
export async function buildMiddleWare(
	...
	for (const [_dstFile, filepath] of Object.entries(mapper!)) {
		...
		await fs.promises
			.mkdir(dirnamePathDst, {recursive: true})
			.then(async () => {
				const check = await checkIsFolder(path);
				return !check ? fs.promises.copyFile(path!, pathDst) : null;
			})
			.catch(async (reason) => {
				// FIXME: no .code in Error interface of nodejs. Need find the right interface.
				if (
					reason.code === ErrorCode.EISDIR ||
					reason.code === ErrorCode.ERR_FS_EISDIR
				)
					return await fs.promises.mkdir(dstFile);
				else if (
					reason.code === ErrorCode.ENOENT &&
					reason.path &&
					reason.dest
				) {
